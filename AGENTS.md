...
### E2E Tests
- **Core functional tests assert non-zero results.** Tests that validate search results (`basic_search`, `response_structure`, `all_safesearch_levels`, `paginations`, `limit_boundaries`, `parameter_combinations`, `unicode_and_special_characters`) use strict `len(Results) > 0` checks. If the live server returns only an infobox with 0 web results, those tests log a `t.Logf` warning instead of failing, and the warnings are collected in a `WARNING SUMMARY` block at the end of each test group (`TestMCPFunctional`, `TestMCPStdioE2E`, `TestCLISmoke`).
...
- **Live-server assertions live in Go E2E tests, never as shell smoke.** Any CI step whose outcome depends on a live SearXNG response must live in a Go E2E test (`-tags=e2e`) so it shares the `WARNING SUMMARY` pattern. The canonical homes are `TestMCPFunctional` (MCP stdio protocol), `TestMCPStdioE2E` (MCP stdio lifecycle + smoke), and `TestCLISmoke` (binary CLI mode: parameter combinations, answer regexes, infobox / language / long-query checks). Shell smoke steps must not use `|| echo` fallbacks, `grep -Eq "A|B"` alternation, `python3 -c` with `assert` that swallows non-zero exits, or any other pattern that masks a missing expected match. If a behavior must be checked from a shell step, the assertion must exit non-zero when the expected evidence is absent; "no evidence but no error" is never an acceptable shell-step outcome for a live-server check. The only shell smoke allowed in `.github/workflows/e2e.yml` are deterministic exit-code / structural checks (e.g. `empty query fails` — empty input is a client-side validation error, not a live-server response). All other live-server checks must be Go E2E.
