...
- **Live-server assertions live in Go E2E tests, never as shell smoke.** Any CI step whose outcome depends on a live SearXNG response must live in a Go E2E test (`-tags=e2e`) so it shares the `WARNING SUMMARY` pattern. Shell smoke steps must not use `|| echo` fallbacks, `grep -Eq "A|B"` alternation, `python3 -c` with `assert` that swallows non-zero exits, or any other pattern that masks a missing expected match. If a behavior must be checked from a shell step, the assertion must exit non-zero when the expected evidence is absent; "no evidence but no error" is never an acceptable shell-step outcome for a live-server check.
