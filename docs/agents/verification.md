...
| `test` | `go test ./...` | Unit tests, excluding e2e and stress |
...
| `lint-drift-check` | (bash script inside justfile) | Drift test ensuring the lint build-tag matrix stays in sync between `justfile` and `.github/workflows/lint.yml` |
