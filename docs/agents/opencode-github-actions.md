...
This rule exists because issue #32 documented an OpenCode investigation where a
GitHub Actions run used a squatted-looking `opencode@users.noreply.github.com`
identity. That kind of value is evidence of a problem, not a source of truth.
...
PR #289 is useful as a PR-body example: it provides a concrete step summary,
verification commands, a linked issue, and supplemental OpenCode/GitHub run
links. Its commit headline, `Config plumbing refactor (A/B+C)`, is concise and
change-focused; that is the useful part of the commit metadata for this guide.
...
- Follow the E2E and test-server boundaries in `docs/agents/e2e-tests.md` and
