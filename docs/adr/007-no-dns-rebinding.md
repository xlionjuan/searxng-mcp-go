...
The redirect SSRF defense in `search.go` blocks redirects to private/internal hosts using `isPrivateHost()`. One potential attack vector is DNS rebinding: a hostname initially resolves to a public IP, then after the host check passes, rebinds to a private IP during the actual TCP dial.
...
3. **Existing mitigations suffice:** The host-level `isPrivateHost()` check, proper host normalization, and DNS resolution in the redirect path already cover the realistic attack surface.
